...
class UNET:
    def __init__(self, batch_size=64, epochs=30, window_size=256, loss='mse'):
        self.batch_size = batch_size
        self.window_size = window_size
        self.epochs = epochs
        self.weight_file = str(Path('best_weight.hdf5'))
        self.model = self.init_network((window_size, window_size, 3))
        self.model.compile(loss=dice_coef_loss if 'dice' else 'mse', optimizer='adam', metrics=['accuracy'])
    ...
    def input_label_generator(self, mode='train'):
        ...
        X_train = ImageDataGenerator(**data_gen_args).flow_from_directory(
            Path('dataset', mode, 'input'), batch_size=self.batch_size, color_mode='rgba',
            seed=SEED)
        y_train = ImageDataGenerator(**data_gen_args).flow_from_directory(
            Path('dataset', mode, 'label'), batch_size=self.batch_size,
