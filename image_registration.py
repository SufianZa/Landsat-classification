...
from skimage.exposure import adjust_gamma, equalize_hist, match_histograms
from string import Template
...
def image_registration(landsat_image_path):
    with rasterio.open(file) as ds:
        with rasterio.open(landsat_image_path) as l_sat:
            ...
            nir = l_sat.read(5)
            m_nir = nir != 0
            nir = (nir - np.min(nir)) / (np.max(nir) - np.min(nir))
            ...
            ls_original = np.dstack((blue, green, red, nir))

            # extract mask from the bands
            mask = np.mean(np.dstack((m_blue, m_green, m_red, m_nir)), axis=2)
            mask[mask > 0] = 1
            mask[mask <= 0] = 0
